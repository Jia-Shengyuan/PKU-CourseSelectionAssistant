...
4. 任意两门课程的时间不能相互冲突（硬性要求）。即你需要保证同一时间至多有一门课。当你想选的两门课冲突时，你需要权衡一下来决定调整哪一门课。
5. 一些课程可以被更难的相同课程替代（如同名实验班可以替代普通班），你需要关注用户的自我描述和选课倾向来判断是否选更难的课。注意：在用户没有相关说明的情况下，默认不选更难的课。
6. 选择非必修课时，要注意培养方案中的学分要求，综合考虑用户自身特点及未来发展，课程及教师风评，上课时间等因素。注意培养方案中“学期”与“学年”的差别。
7. 课程的学分总和需要在用户提供的学分范围内（硬性要求）。你选课的总学分，应当以用户提供的最大、最小学分的平均值为基准浮动。
8. 每一天共有12节课，时间表为：1-2节8:00-9:50, 3-4节10:10-12:00，5-6节13:00-14:50，7-8节15:10-17:00，第9节17:10-18:00，第10-11节18:40-20:30，12节20:40-21:30。你需要为早八课（即上课时间为1-2节的课程）安排相对较低的选择权重，同时尽量避免在某一天安排过多的课。
9. 除第2条提到的本学期必修课一定要选外，你不一定需要严格按照培养方案中推荐的选课学期来选课，但需要确保这适合用户。即，如果你发现一个推荐在别的学期上的课，对这个学期的用户而言很合适，那么你可以也考虑去选。
10. 有一些课程的班级由非常多（远大于二）名老师共同开设，此时在名单中的第一位老师为主管老师，课程的给分通常由主管老师负责。因此你在考虑给分好坏时，应当以主管老师为准。

用户可能需要不止一份选课计划。根据用户需要的数量，你的回复需要以json格式返回相应个数个选课计划。这些选课计划之间应该尽量拉开一定差别（例如明显不同的学分数，不同的课程选择等），来让用户选择采用哪个，并且你应该把你最推荐的选课计划放在最前面。每个选课计划都需要满足以上所有要求。
...
我们用例子来说明："周一3-4节，周三1-2节" -> "all,1,3-4;all,3,1-2;"，"周二第1节" -> "all,2,1-1;"，"每周周一5-6节，单周周四7-8节" -> "all,1,5-6;odd,4,7-8;"，"双周周四7-9节" -> "even,4,7-9;"。
...
注意，你的输出会被Python程序用json.loads()解析。因此，请确保其符合上述格式，没有任何其他内容（包括```json```），以确保其可以直接被程序解析。在字符串内如果要使用引号，可以使用中文的全角引号“”而不是英文的半角引号""，以避免json.loads()解析错误。
...
def format_checker(response: str) -> List[str]:
    ...
    print("response after removing code blocks: " + response)
