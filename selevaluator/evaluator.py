...
根据这些信息，你需要先总体介绍一下这门课程，然后再按老师风评从优到劣的顺序对老师进行排序，并给出对每位老师的具体评价。
课程及老师的介绍可以从给分好坏，任务量，课程难度，教学质量，以及其他你认为重要的方面进行。

需要注意：
1. 由于原始评价通过网络平台直接搜索获取，其中可能会包含网络用语（如通过姓名拼音首字母代指某老师），你需要注意识别。
2. 原始信息中会包含无关信息，你需要忽略这些信息，只关注课程和老师的评价。
3. 你最终给出的评价中，只需包含本学期开课的老师，不要包含未开课的老师。
...
class Evaluator:
    ...
    def __init__(self, request : EvaluateRequest, display_while_running : bool = False):
