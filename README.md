...
- 保存好你的密钥，不要泄露
- 新用户通常自带一定使用额度，但只能使用标准版模型（通常来说够用了）。但若要使用 Pro 模型（支持更高的访问量），或者超出额度，你可能需要实名认证并充值（这样的服务当然是花钱的！）
...
填写完成后，点击“开始搜索推荐”，程序就会开始自动爬取树洞课程，以及大模型汇总选课等一系列过程。第一次运行会自动检索并下载部分依赖，可能较慢，请耐心等待。**如果你是第一次运行，或者与上一次运行之间间隔了很长的时间，你需要手动在弹出的树洞界面中登录。**
